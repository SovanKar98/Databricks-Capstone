...
file = spark.read.text(batch_2017_path)
file = file.withColumn("x",col("value")).drop("value")
file.schema
...
def func(inp):
  d = fixed_width_column_defs
  ret = []
  for key in d:
    st = d[key][0] - 1
    end = d[key][0] - 1 + d[key][1]
    ret.append((inp[st:end]).strip())
  #ret = ( inp[fixed_width_column_defs[key][0]-1: fixed_width_column_defs[key][0]-1 + fixed_width_column_defs[key][1]] for key in fixed_width_column_defs)
  return (tuple(ret))
rdd = spark.sparkContext.textFile(batch_2017_path)
headers = list(fixed_width_column_defs.keys())
 
schema = StructType([ StructField(x,StringType(),True) for x in headers])
#schema = " string,".join(fixed_width_column_defs) + ' string'
rdd1 = rdd.map(lambda x: func(x))
df = spark.createDataFrame(rdd1,schema).na.replace("",None).na.replace("null",None).withColumn('ingest_file_name',input_file_name()).withColumn('ingested_at',current_timestamp())
...
df = spark.read.option('sep','\t').option('header','true').csv(batch_2018_path)
df = df.na.replace("",None).na.replace("null",None).withColumn('ingest_file_name',input_file_name()).withColumn('ingested_at',current_timestamp())
...
headers = list(fixed_width_column_defs.keys())
schema = StructType([ StructField(x,StringType(),True) for x in headers])
df = spark.read.option('sep',',').option('header','true').schema(schema).csv(batch_2019_path)
df = df.na.replace("",None).na.replace("null",None).withColumn('ingest_file_name',input_file_name()).withColumn('ingested_at',current_timestamp())
...
x = spark.read.format("delta").load(batch_target_path)
display(x)
