...
df1 = df.select(
  col("_product_id").alias("product_id"),"color","model_name","model_number",
  col("price._base_price").alias("base_price"),
  col("price._color_adj").alias("color_adj"),
  col("price._size_adj").alias("size_adj"),
  col("price.usd").alias("price"),
  "size")

# COMMAND ----------

df2 = df1.filter(col("price").isNotNull())
...
(df2.write
  .format("delta")
  .mode("overwrite")
  .saveAsTable(f"{user_db}.{products_table}")
)
