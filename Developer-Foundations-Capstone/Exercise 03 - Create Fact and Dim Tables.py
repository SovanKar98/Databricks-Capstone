...
x = spark.read.format("delta").load(batch_source_path)
x.createOrReplaceTempView(batch_temp_view)
...
df1 = spark.sql("select * from batched_orders")
df2 = df1.drop("submitted_at", "order_id", "customer_id","shipping_address_attention", "shipping_address_address", "shipping_address_city", "shipping_address_state", "shipping_address_zip","product_id", "product_quantity", "product_sold_price")
df3 = df2.withColumn("_error_ssn_format",  col("sales_rep_ssn").contains('-') )
df4 = df3.withColumn("sales_rep_ssn", regexp_replace( col("sales_rep_ssn"), "-","" ).cast("Long"))
df5 = df4.withColumn("sales_rep_zip",col("sales_rep_zip").cast("int")).dropDuplicates(['sales_rep_id','sales_rep_ssn','sales_rep_first_name','sales_rep_last_name','sales_rep_address'])
loc = 'dbfs:/user/infy_1001461@outlook.com/dbacademy/developer-foundations-capstone/sales_reps.delta'
df5.write.format("delta").mode('overwrite').option("overwriteSchema",True).saveAsTable(f"{user_db}.{sales_reps_table}")
#spark.sql(f"CREATE TABLE sales_reps USING DELTA LOCATION '{loc}'")

# COMMAND ----------

a=spark.sql(f"select * from {batch_temp_view} where sales_rep_id = 'ef5ef0a4-3d1a-43af-8ef4-2919ddcfdfec'")
spark.read.table(sales_reps_table).filter("_error_ssn_format == false").count()
...
db_batch = spark.sql(f"select * from {batch_temp_view}")\
                ...
                .drop("sales_rep_ssn", "sales_rep_first_name", "sales_rep_last_name", "sales_rep_address",\
                      "sales_rep_city", "sales_rep_state", "sales_rep_zip","product_id", "product_quantity",\
                      "product_sold_price")\
...
db_batch.write.format("delta").mode("overwrite").partitionBy("submitted_yyyy_mm").saveAsTable(f"{orders_table}")
...
db_batch = spark.sql(f"select * from {batch_temp_view}")\
                .select("order_id","product_id","product_quantity","product_sold_price","ingest_file_name"\
                        ,"ingested_at")\
                ...
                .write.format("delta").mode("overwrite").saveAsTable(f"{line_items_table}")
