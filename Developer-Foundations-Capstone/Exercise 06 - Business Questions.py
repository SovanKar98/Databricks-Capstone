...
df = spark.sql("select shipping_address_state, count(*) count from orders\
...
# MAGIC %sql
# MAGIC select  avg(product_sold_price), min(product_sold_price), max(product_sold_price) from line_items li
# MAGIC join orders o on li.order_id = o.order_id
# MAGIC join products p on li.product_id = p.product_id
# MAGIC join sales_reps sr on o.sales_rep_id = sr.sales_rep_id
# MAGIC where p.color = 'green' and o.shipping_address_state = 'NC' and _error_ssn_format=true

# COMMAND ----------

df = spark.sql("select  avg(product_sold_price), min(product_sold_price), max(product_sold_price) from line_items li\
 join orders o on li.order_id = o.order_id\
 join products p on li.product_id = p.product_id\
 join sales_reps sr on o.sales_rep_id = sr.sales_rep_id\
 where p.color = 'green' and o.shipping_address_state = 'NC' and _error_ssn_format=true")
...
ex_avg = df[0]["avg(product_sold_price)"]
ex_min = df[0]["min(product_sold_price)"]
ex_max = df[0]["max(product_sold_price)"]
...
df = spark.sql("\
 ...
 from (  select sales_rep_first_name, sales_rep_last_name, (product_sold_price - price) * product_quantity as total_profit\
         from line_items li\
         join orders o on li.order_id = o.order_id\
         join products p on li.product_id = p.product_id\
         join sales_reps sr on o.sales_rep_id = sr.sales_rep_id) a group by sales_rep_first_name, sales_rep_last_name order by 3 desc limit 1")
