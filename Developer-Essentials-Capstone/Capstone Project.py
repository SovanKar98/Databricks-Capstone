...
def loadStaticData(path):
  df = spark.read.format("delta").load(lookupSourcePath).filter(col("deviceType").isin("android","ios")).dropDuplicates(["device_id"])
  return df
...
tempdf= spark.read.format("delta").load(outputPathBronzeTest).withColumn("device_id",col("eventParams.device_id")).withColumn("client_event_time",col("eventParams.client_event_time")).join(deviceLookupDF,"device_id","inner").drop("eventParams").select("device_id","eventName","client_event_time","eventDate","deviceType")
display(tempdf)

# COMMAND ----------

working_dir = f"dbfs:/dbacademy/sovan.kar@wipro.com/developer-essentials-capstone"
...
def bronzeToSilver(bronzePath, silverPath, streamName, lookupDF):
  (spark.readStream
    .format("delta")
    .load(bronzePath)
.withColumn("device_id",col("eventParams.device_id")).withColumn("client_event_time",col("eventParams.client_event_time")).join(lookupDF,"device_id","inner").drop("eventParams").select("device_id","eventName","client_event_time","eventDate","deviceType")
...
gold_DF=spark.read.format("delta").load(outputPathSilverTest).withColumn("week",weekofyear(col("eventDate"))).groupBy(col("week")).agg(approx_count_distinct(col("device_id"),rsd=.01).alias("WAU")).select("WAU","week")
display(gold_DF)
...
def silverToGold(silverPath, goldPath, queryName):       (spark.readStream.format("delta").load(silverPath).withColumn("week",weekofyear(col("eventDate"))).groupBy(col("week")).agg(approx_count_distinct(col("device_id"),rsd=.01).alias("WAU")).writeStream 
    .format("delta")        
...
spark.sql(f"""CREATE TABLE IF NOT EXISTS mobile_events_delta_gold using delta location "{outputPathGoldTest}" """)
